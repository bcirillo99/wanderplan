...
from travel_planner.db.models import Trip, Flight, Transport, Accommodation, Activity, Extra
...
def get_trip_cost_summary(db: Session, trip_id: UUID) -> dict:
    ...
    extras = db.query(Extra).filter(Extra.trip_id == trip_id).all()
    ...
    extra_total = sum(e.amount for e in extras if e.amount)

    total = flight_total + transport_total + accommodation_total + activity_total + extra_total
